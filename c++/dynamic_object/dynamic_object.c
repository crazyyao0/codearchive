...
#include <stdlib.h>
	...
	#define free   SWFREE
	#define calloc SWCALLOC
	#define realloc SWREALLOC
#else
	#include <malloc.h>
#endif
...
dynamic_object* dynamic_string_append(dynamic_object*obj, char* s, int slen)
{
	...
	if(max != obj->string_value->max)
	{
		...
		newstr->str[obj->string_value->len] = 0;
		free(obj->string_value);
		obj->string_value = newstr;
	}

	memcpy(obj->string_value->str + obj->string_value->len, s, slen);
	obj->string_value->len = len;
	obj->string_value->str[len] = 0;
	return obj;
}

dynamic_object* dynamic_set_string(dynamic_object*obj, char* s)
{
	dynamic_set_nstring(obj, s, strlen(s));
	return obj;
}
...
dynamic_object* dynamic_set_nstring(dynamic_object*obj, char* s, int slen)
{
	dynamic_set_null(obj);
	obj->type = DYNAMIC_TYPE_STRING;
	obj->string_value = (dynamic_string*)malloc(sizeof(dynamic_string) + slen);
	obj->string_value->len = slen;
	obj->string_value->max = slen;
	memcpy(obj->string_value->str, s, slen);
	obj->string_value->str[slen]= 0;
	return obj;
}
...
dynamic_object* dynamic_string_printf(dynamic_object*obj, char* s, ...)
{
	...
	va_start(args, s);
	vsnprintf(obj->string_value->str, slen + 1, s, args);
	va_end(args);
	obj->string_value->str[slen]= 0;
	return obj;
}
...
static unsigned int dynamic_hash_string(char* str)
{
...
static void dynamic_hash_insert(dynamic_hash_table* table, dynamic_hash_entry* entry)
{
...
static dynamic_hash_entry* dynamic_hash_find(dynamic_hash_table* table, char* key)
{
...
dynamic_object* dynamic_map_insert(dynamic_object* map, char* key)
{
	...
	newentry->value.type = DYNAMIC_TYPE_NULL;
...
char* dynamic_map_get_value(dynamic_object* map, char* key)
{
	...
	if(map->type != DYNAMIC_TYPE_MAP)
		return 0;
	e = dynamic_hash_find(map->map_value, key);
	if(e == 0 || e->value.type != DYNAMIC_TYPE_STRING)
		return 0;
	return e->value.string_value->str;
}
...
int dynamic_as_string(dynamic_object* obj, char* buf, int len)
{
	...
	case DYNAMIC_TYPE_STRING:
		return snprintf(buf, len, "%s", obj->string_value->str);
	case DYNAMIC_TYPE_BYTES:
...
int dynamic_size(dynamic_object* obj)
{
	...
	case DYNAMIC_TYPE_BOOL:
		return 1;
	case DYNAMIC_TYPE_INTEGER:
		if(obj->int_value | 0xFFFFFFFFFFFFFF00LL)
			return 1;
		else if(obj->int_value | 0xFFFFFFFFFFFF0000LL)
			return 2;
		else if(obj->int_value | 0xFFFFFFFF00000000LL)
			return 4;
		else
			return 8;
	case DYNAMIC_TYPE_DOUBLE:
