...
HTTP_ERROR_CODE g_http_errorcode[] = {
...
{
	int low=0;
    int high = sizeof(g_http_errorcode)/sizeof(HTTP_ERROR_CODE) - 1;

    while(low<=high)
    {
        int mid = (low + high)/2;
        HTTP_ERROR_CODE *t = g_http_errorcode + mid;
		if(code == t->code)
            return t;
		else if(code < t->code)
            high = mid - 1;
	...
	return g_http_errorcode + 34;
}

HTTP_MIME_MAP* http_find_mime(char* filename)
{
	char* fileext = strrchr(filename, '.');
	int low=0;
    int high = sizeof(g_http_mime_map)/sizeof(HTTP_ERROR_CODE) - 1;

	if(fileext == 0)
		return g_http_mime_map + high;
	fileext ++;

    while(low<=high)
    {
        int mid = (low + high)/2;
		HTTP_MIME_MAP *t = g_http_mime_map + mid;
		int cmp = strcmp(fileext, t->fileExt);
		if(cmp == 0)
            return t;
		else if(cmp < 0)
            high = mid - 1;
	...
	return g_http_mime_map + sizeof(g_http_mime_map)/sizeof(HTTP_ERROR_CODE) - 1;
}